...
/**
 * 字段名保真测试：canonical 是 Go（Elysia-Api）与 TS 共享的线上规范，
 * JSON 字段名必须与 canonical.go 的 json 标签完全一致（snake_case），
 * golden fixture 才能在两侧测试里通用。
 */
describe('canonical 类型 JSON 字段名保真', () => {
  ...
  it('JSON.parse 的线上对象可直接断言为 canonical 类型（无映射层）', () => {
    const wire = `{
    ...
    const response: CanonicalResponse = JSON.parse(wire)
